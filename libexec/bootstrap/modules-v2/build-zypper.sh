...
export ZYPP_CONF
ZYPP_REPO="/etc/zypp/repos.d/base.repo"
export ZYPP_REPO
...
echo "[base]" > "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo 'name=Linux $releasever - $basearch' >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "baseurl=$MIRROR" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "enabled=1" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "gpgcheck=0" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"

if [ ! -z "${MIRROR_UPDATES:-}" ]; then
echo "[updates]" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo 'name=Linux $releasever - $basearch updates' >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "baseurl=${MIRROR_UPDATES}" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "enabled=1" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
echo "gpgcheck=0" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"
fi

echo "" >> "$SINGULARITY_ROOTFS/$ZYPP_REPO"

if ! eval "$INSTALL_CMD -c $SINGULARITY_ROOTFS/$ZYPP_CONF --root $SINGULARITY_ROOTFS --gpg-auto-import-keys -n install --auto-agree-with-licenses $RELEASE_PACKAGE coreutils $INSTALLPKGS"; then
